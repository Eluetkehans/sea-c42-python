...
# the total sum of C, G, T, and A. Some Ns are in the sequences.
cgta_total = g_count + c_count + a_count + t_count


# divide the gc_count by the total_count
gc_content = float(gc_count) / cgta_total


# divide the at_count by the total_count
at_content = float(at_count) / cgta_total


# divide the at_count by the cg_count
at_gc_ratio = float(at_count / gc_count)


# Determine the GC classification
def gc_class():
    if(gc_content >= .6):
